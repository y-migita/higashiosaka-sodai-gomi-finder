...
bun run fetch               # 公開サイトから data/categories.json・items.json を取得/更新（任意）
bun run index               # 全248件を埋め込み data/index.json を生成（初回 & データ更新時のみ）
bun run search "<検索語>"    # 検索。既定はJSON出力（LLM利用前提）
```

- 検索オプション: `--k <件数>`（既定5） / `--pretty`（人間向け・罫線テーブル表示）。
- ログ/整形表示の色付けは `src/term.ts`（依存ゼロ）。`NO_COLOR`・非TTY（パイプ）で自動無効化。
...
この2ファイルは `bun run fetch`（`src/fetch-data.ts`）で同サイトから再取得できる。品目検索ページが内部で叩く Teeda Ajax（`teeda.ajax`）を再現し、分類は `action=ajaxGetClassList`、全品目は `action=ajaxGetDustAllList` で取得する。出力は既存フォーマット厳守（categories=コンパクト / items=4スペース整形）なので、サイト無変更なら再取得しても差分は出ない。
...
├── term.ts         ターミナル表示（依存ゼロ）: ANSI色 + 全角対応テーブル。人間向け表示専用
├── fetch-data.ts   bun run fetch のエントリ（teeda.ajax から data/*.json を取得）
...
- `data/categories.json` / `data/items.json` は**手で編集しない**。更新は `bun run fetch` で公開サイトから取り込む（出典どおりのデータを保つため）。
