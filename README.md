...
データ出典は、東大阪市が運営する公式サイト **[東大阪市大型ごみインターネット受付](https://www.ogomi-higashiosaka.jp/eco/view/higashiosaka/top.html)** の品目情報です。

## データ

データの出典は **[東大阪市大型ごみインターネット受付](https://www.ogomi-higashiosaka.jp/eco/view/higashiosaka/top.html)**（運営: 東大阪市）。
同サイトは大型ごみ収集の申込み・変更・取消や品目検索を提供する公式サービスで、本CLIはその品目データ（分類・料金・処理券枚数）をローカル検索できるようにしたもの。
実際の収集申込みは上記サイト、または大型ごみ受付センター（☎ 072-962-5374）へ。
