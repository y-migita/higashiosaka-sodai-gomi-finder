...
async function main() {
  ...
  console.error(
    c.gray(`items ${items.length}件 / categories ${classes.size}件 を読み込みました。`),
  );
  ...
  console.error(c.bold(c.cyan("dustType → 分類 の対応:")));
  for (const [t, e] of [...typeMap].sort((a, b) => a[0] - b[0])) {
    console.error(
      c.gray(`  dustType=${t} → ${e.category}  (${e.count}件)`),
    );
  }
  ...
  console.error(
    c.cyan(`埋め込みを生成中… (model=${EMBED_MODEL}, ${prepared.length}件)`),
  );
  ...
  console.error(
    c.green(`✔ index.json を生成しました（${indexed.length}件 / ${mb}MB）`),
  );
}

main().catch((e) => {
  console.error(c.red(`✖ index 生成に失敗: ${e?.message ?? e}`));
  process.exit(1);
});
